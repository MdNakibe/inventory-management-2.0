<template>
    <main>
        <div class="flex justify-between">
            <span class="mb-5 text-2xl text-slate-700 dark:text-white font-bold">Cutomer Payment Record</span>
            <div>
                <router-link to="/customers-payment/receive-new-payment">
        ...
        <div class="grid justify-between grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 2xl:gap-28 items-end bg-white dark:bg-slate-700 p-5 rounded-lg shadow">
            <div class="grid w-full gap-2 col-span-1 dark:text-white">
                ...
                <input type="text" class="w-full dark:bg-slate-800" autofocus="true" placeholder="Select/Search Customer">
            </div>
            ...
            <table class="w-full dark:text-white">
                ...
                <tr v-for="(item, ind) in this.$store.getters['supplier/supplierPaymentRecord']" :key="ind" class="hover:bg-slate-100 dark:hover:bg-slate-600">
                    ...
                    <vtd class="flex justify-center">
                        ...
                        <button title="Edit" @click="$router.push(`/customers-payment/edit-payment/${item.id}`)" class="hover:bg-[#3056d3] py-0.5 px-1.5 bg-[#3056d3]/10 rounded text-[#3056d3] hover:text-white mr-1">
...
export default{
    ...
    async created() {
        await this.$store.dispatch('supplier/getSupplierPaymentRecord',{type: 2});
    },
}
</script>