<template>
    <main>
        <div class="flex justify-between items-center mb-5">
            <div class="text-2xl text-slate-700 dark:text-white font-bold">Make Customer Payment</div>
        ...
        <div class="bg-white dark:bg-slate-700 dark:text-white mt-5 rounded-lg p-5">
            <table class=" w-full">
                ...
                <tr v-for="(due, ind) in cart" :key="ind" class="hover:bg-slate-100 dark:hover:bg-slate-600">
                    ...
                    <vtd class="text-left">{{ due.invoice }} - {{ due.customer.name }}</vtd>
...
export default{
    ...
    async created() {
        ...
        this.cart = this.$store.getters['customer/customerdue'];
            this.cart.map(item => {
                return item;
            })
    },
