<template>
    <main>
        <div class="flex justify-between">
            <span class="mb-5 text-2xl text-slate-700 dark:text-white font-bold"> Sales Return</span>
            <router-link to="/sales-return/record-sales-return">
        ...
        <div class="bg-white dark:bg-slate-700 mt-5 rounded-lg p-5 dark:text-white">
            <DataView class="mt-5" :value="filteredReturn" dataKey="id"
...
export default{
    ...
    async created() {
        await this.$store.dispatch('sale/getSaleReturn');
    },
    computed: {
        filteredReturn() {
            const sale = this.$store.getters['sale/saleReturn'];
            return sale;
        },
    },
}
